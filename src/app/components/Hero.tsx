...
export default function Hero({ headline, subheadline, ctaText, heroImageUrl, onCtaClick } : { headline: string; subheadline: string; ctaText: string; heroImageUrl: string; onCtaClick: () => void; }) {
  return (
    <header id="hero" className="relative isolate overflow-hidden bg-background flex items-stretch -mt-20">
      ...
      <div className="relative mx-auto flex flex-col items-center justify-center w-full max-w-3xl px-6 py-16 text-[#3a2c1a] sm:px-10 min-h-screen">
        <h1 className="font-kitchen text-5xl sm:text-6xl lg:text-7xl font-bold text-center leading-tight text-[#3a2c1a] drop-shadow-md">
          {headline}
        </h1>
        ...
        <button
          type="button"
          onClick={onCtaClick}
          className="mt-10 inline-flex items-center rounded-full bg-[#bfa76a] px-10 py-3 text-base font-semibold text-white shadow-lg shadow-[#bfa76a]/20 transition hover:bg-[#a88c4a] focus:outline-hidden focus-visible:ring-2 focus-visible:ring-[#e7cfa4]"
        >
          {ctaText}
        </button>
      </div>
